...
<div class="container mt-4">
    ...
    <div class="card bg-dark text-white border-neon p-4">
        ...
        @if (($task->created_by == auth()->user()->id || auth()->user()->is_manager || $task->users->contains(auth()->user())) && $task->status != 'approved' && $task->status != 'completed')

            <a href="{{ route('tasks.edit', $task->id) }}" class="btn btn-primary btn-sm w-100 neon-btn border-neon neon-link mt-3">Editar</a>

        @elseif ($task->status == 'completed' && auth()->user()->is_manager)

            <a href="{{ route('tasks.showReview', $task->id) }}" class="btn btn-primary btn-sm w-100 neon-btn border-neon neon-link mt-3">Aprovar</a>

        @endif

        <a href="{{ route('tasks.index') }}" class="btn btn-outline-light btn-sm w-100 neon-btn border-neon mt-3">Voltar</a>
    </div>
</div>
@endsection
